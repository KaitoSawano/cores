...
import (
	...
	"github.com/xcosh/go-xcosh/signer/core/apitypes"
)
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useXcoshV = true // Default to use V = 27 or 28, the legacy Xcosh format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useXcoshV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Xcosh PoA standard
	...
	default: // also case TextPlain.Mime:
		// Calculates an Xcosh ECDSA signature for:
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// https://geth.xcosh.org/docs/tools/clef/apis#account-ecrecover
	if len(sig) != 65 {
