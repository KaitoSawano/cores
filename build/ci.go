...
import (
	...
	"github.com/xcosh/go-xcosh/crypto/signify"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Xcosh contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Xcosh bootnode.",
		},
		{
		...
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Xcosh Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "geth",
			Description: "Xcosh CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Xcosh account management tool.",
		},
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/xcosh/go-xcosh/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/xcosh/go-xcosh/internal/version.gitDate="+env.Date)
	}
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/xcosh/execution-spec-tests/releases/download/v%s/%s%s", executionSpecTestsVersion, base, ext)
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "xcosh/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - xcosh/client-go:latest                            - Pushes to the master branch, Geth only
	//  - xcosh/client-go:stable                            - Version tag publish on GitHub, Geth only
	//  - xcosh/client-go:alltools-latest                   - Pushes to the master branch, Geth & tools
	//  - xcosh/client-go:alltools-stable                   - Version tag publish on GitHub, Geth & tools
	//  - xcosh/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Geth only
	//  - xcosh/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Geth & tools
	//  - xcosh/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Geth only
	//  - xcosh/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Geth & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "xcosh/xcosh")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-xcosh sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "xcosh"
...
type debMetadata struct {
	...
	// go-xcosh version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Xcosh Builds <fjl@xcosh.org>"
	}
	return debMetadata{
