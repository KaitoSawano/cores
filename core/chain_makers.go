...
func newChainMaker(bottom *types.Block, config ctypes.ChainConfigurator, engine consensus.Engine) *chainMaker {
	return &chainMaker{
		bottom:      bottom,
		config:      config,
		engine:      engine,
...
func (cm *chainMaker) Config() ctypes.ChainConfigurator {
	return cm.config
}
