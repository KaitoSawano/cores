...
// Prefetch processes the state changes according to the Xcosh rules by running
...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.StateDB, cfg vm.Config, interrupt *atomic.Bool) {
	...
	byzantium := p.config.IsEnabled(p.config.GetEIP161abcTransition, block.Number())
...
func precacheTransaction(msg *Message, config ctypes.ChainConfigurator, gaspool *GasPool, statedb *state.StateDB, header *types.Header, evm *vm.EVM) error {
