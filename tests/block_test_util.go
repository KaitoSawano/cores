...
// Package tests implements execution of Xcosh JSON tests.
package tests
...
type btJSON struct {
	...
	Post       genesisT.GenesisAlloc `json:"postState"`
...
func (t *BlockTest) Run(snapshotter bool, scheme string, tracer vm.EVMLogger, postCheck func(error, *core.BlockChain)) (result error) {
	...
	gblock, err := core.CommitGenesis(gspec, db, triedb)
	if err != nil {
		return err
	}
...
func (t *BlockTest) genesis(config ctypes.ChainConfigurator) *genesisT.Genesis {
	if config == nil {
...
/*
See https://github.com/xcosh/tests/wiki/Blockchain-Tests-II
