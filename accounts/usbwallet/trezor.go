...
import (
	...
	"github.com/xcosh/go-xcosh/accounts/usbwallet/trezor"
...
// or whether the Xcosh app was not started on it.
...
// Xcosh address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.XcoshAddress)
	if _, err := w.trezorExchange(&trezor.XcoshGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.XcoshTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.XcoshTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
