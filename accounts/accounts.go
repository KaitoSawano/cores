...
// Package accounts implements high level Xcosh account management.
package accounts
...
// Account represents an Xcosh account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Xcosh account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain xcosh.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Xcosh Signed Message:\n%d%s", len(data), string(data))
