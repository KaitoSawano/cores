...
import (
	...
	"github.com/xcosh/go-xcosh/accounts/scwallet"
	...
	"github.com/xcosh/go-xcosh/eth/gasestimator"
	"github.com/xcosh/go-xcosh/eth/tracers/logger"
...
// XcoshAPI provides an API to access Xcosh related information.
type XcoshAPI struct {
	b Backend
}

// NewXcoshAPI creates a new Xcosh protocol API.
func NewXcoshAPI(b Backend) *XcoshAPI {
	return &XcoshAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *XcoshAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *XcoshAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *XcoshAPI) FeeHistory(ctx context.Context, blockCount math.HexOrDecimal64, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *XcoshAPI) Syncing() (interface{}, error) {
...
// XcoshAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type XcoshAccountAPI struct {
	am *accounts.Manager
}

// NewXcoshAccountAPI creates a new XcoshAccountAPI.
func NewXcoshAccountAPI(am *accounts.Manager) *XcoshAccountAPI {
	return &XcoshAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *XcoshAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Xcosh ECDSA signature for:
// keccak256("\x19Xcosh Signed Message:\n" + len(message) + message))
//
...
//
// https://geth.xcosh.org/docs/interacting-with-geth/rpc/ns-personal#personal-sign
...
// the address of:
// hash = keccak256("\x19Xcosh Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://geth.xcosh.org/docs/interacting-with-geth/rpc/ns-personal#personal-ecrecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, errors.New("invalid Xcosh signature (V is not 27 or 28)")
	}
...
// BlockChainAPI provides an API to access Xcosh blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Xcosh blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Xcosh chain config.
//
...
//
// https://github.com/xcosh/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Xcosh APIs exposed over the debugging
// namespace.
...
// Version returns the current xcosh protocol version.
