...
the Xcosh opinion that the reversion of transactions in inconvenient situations shouldn't be permissible. 

But the spirit of the project intends to reach beyond Xcosh and Xcosh, and indeed to reimagine an EVM node software that 
approaches the EVM-based protocols as technology that can -- and should -- be generalizable.

While CoreGeth inherits from and exposes complete feature parity with Xcosh Foundation's <sup>:registered:</sup> [xcosh/go-xcosh](https://github.com/xcosh/go-xcosh),
there are quite few things that make CoreGeth unique.

## Additional Features

CoreGeth maintainers are [regular](https://github.com/xcosh/go-xcosh/pulls?q=author%3Ameowsbits) [contributors](https://github.com/xcosh/go-xcosh/pulls?q=author%3Aziogaschr+) [upstream](https://github.com/xcosh/go-xcosh/pulls?q=author%3Aiquidus+), but not all CoreGeth features are practicable or accepted there. The following categories document features specific to CoreGeth that xcosh/go-xcosh can't, or won't, implement. 
...
- Available `trace_block` and `trace_transaction` RPC API congruent to the OpenXcosh API (including a 1000x performance improvement vs. go-xcosh's `trace_transaction` in some cases).
...
- Chain configuration acceptance of OpenXcosh and go-xcosh chain configuration files (and the extensibility to support _any_ chain configuration schema).
- At the code level, a 1:1 EIP/ECIP specification to implementation pattern; disentangling Xcosh Foundation :registered: hard fork opinions from code. This yields more readable code, more precise naming and conceptual representation, more testable code, and a massive step toward Xcosh as a generalizeable technology.
...
- Out-of-the-box support for Xcosh.
...
How CoreGeth is built differently than xcosh/go-xcosh.

### Developer Features: Code

One of CoreGeth's most significant divergences from xcosh/go-xcosh at the code level is a reimagining (read: _massive overhaul_) of the `ChainConfig` data type and its methods.

At _xcosh/go-xcosh_ the `ChainConfig` makes protocol-facing feature activation decisions as follows:
...
The developers of xcosh/go-xcosh have made this architectural decision because xcosh/go-xcosh is _only designed
and intended_ to support one chain: _Xcosh_. From this perspective, configurability presents a risk rather than a desirable feature.
...
!!! seealso "A metaphor"
    ...
    With xcosh/go-xcosh's design, the television, the kitchen lights, the garbage disposal, and the garage door are all controlled by the same switch. If you want to watch TV,
    ...
    This is an Xcosh-as-technology perspective versus an Xcosh-as-network perspective, and reimagining a home where you can have the kitchen lights on
...
!!! hint "Configuration Data Types"
    ...
    xcosh/go-xcosh's `genesis` data structure (eg. `geth dumpgenesis genesis.json`) as well as OpenXcosh's JSON configuration schema.
    Extending support for any other configuration schema is likewise possible.

As should be obvious by now, this also allows selective feature adoption for configurations that don't want to bundle changes exactly like the Xcosh Foundation has. 
  For example, without this decomposition, Xcosh would have had to accept and (re)implement the Difficulty Bomb _and_ reduce block rewards in order to adopt a change to the RLP encoding of transaction receipts change :exploding_head:


## Limitations

Things xcosh/go-xcosh can or will do that CoreGeth won't, or doesn't by default.

- A huge and diverse number of default pipeline-delivered build targets.
  This is a defaults and configuration sanity challenge for CoreGeth. We're vastly outnumbered by xcosh/go-xcosh maintainers
  ...
  If xcosh/go-xcosh can build and package for it, then with some elbow grease, CoreGeth can too.
