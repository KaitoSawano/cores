...
// Verify that Client implements the xcosh interfaces.
var (
	_ = xcosh.ChainReader(&Client{})
	_ = xcosh.TransactionReader(&Client{})
	_ = xcosh.ChainStateReader(&Client{})
	_ = xcosh.ChainSyncReader(&Client{})
	_ = xcosh.ContractCaller(&Client{})
	_ = xcosh.GasEstimator(&Client{})
	_ = xcosh.GasPricer(&Client{})
	_ = xcosh.LogFilterer(&Client{})
	_ = xcosh.PendingStateReader(&Client{})
	// _ = xcosh.PendingStateEventer(&Client{})
	_ = xcosh.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Xcosh Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new xcosh service: %v", err)
	}
...
func testTransactionInBlock(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != xcosh.NotFound {
		t.Fatal("error should be xcosh.NotFound")
	}
...
func mustNewTestBackend() (*node.Node, []*types.Block) {
	...
	ethservice, err := eth.New(n, config)
	if err != nil {
		panic(fmt.Sprintf("can't create new xcosh service: %v", err))
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
